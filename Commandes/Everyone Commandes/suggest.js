...
module.exports.run = async (bot, message, args) => {
    ...
    const suggestEmbed = new Discord.MessageEmbed()
      ...
      const chan = message.guild.channels.cache.find(c => c.id === '703738705096998912');
      const sendembed = await chan.send(suggestEmbed)
      await sendembed.react('✅');
      await sendembed.react('⛔');
      message.guild.channels.cache.find(c => c.id === '703738705096998912').send(`Commande réussie : **Votre Suggestion à bien était envoyer** !\n\`Ce message ce suprimera automatiquement au bout de 10s!\``).then(m => m.delete({timeout: 10000}));
};
